...
csv_s3_object_prefix = 'data/real_csv'

# Symlink 文件
symlink_s3_bucket = 'emr-bjs-9471'
symlink_s3_object_prefix = 'data/databases/ods_govern/pupumall_dc_sandbox_use'
...
def write_s3(batch_id, batches_count, s3, data):
    ...
    symlink_path_local = "symlink.tmp"
    ...
    for key, value in data.items():
        ...
        # 上传 csv 文件到 S3
        csv_s3_object_key = f"{csv_s3_object_prefix}/{run_id}_{uuid.uuid4()}.csv"
        ...
        # 写 symlink 文件
        with open(symlink_path_local, 'a', encoding='utf-8') as file:
            file.writelines(f"s3://{csv_s3_bucket}/{csv_s3_object_key}")
            file.flush()
        
        # 上传 symlink 文件到 S3
        symlink_s3_object_key = f"{symlink_s3_object_prefix}/dt={key}/symlink.txt"
        s3.upload_file(symlink_path_local, symlink_s3_bucket, symlink_s3_object_key)

        # 删除本地 csv 文件
        os.remove(symlink_path_local)
